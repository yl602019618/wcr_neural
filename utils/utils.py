...
def get_memory(type):
    ...
    if type == "MB":
        ...
        # print(f"Physical memory used: {mem_info.rss / 1024 / 1024} MB")
        # print(f"Virtual memory used: {mem_info.vms / 1024 / 1024} MB")
    ...
    if type == "GB":
        ...
        # print(f"Physical memory used: {mem_info.rss / 1024 / 1024 / 1024} GB")
        # print(f"Virtual memory used: {mem_info.vms / 1024 / 1024 / 1024} GB")
...
def plot_loss(loss_list, n=0, ylabel=None, title=None, filename=None):
    plt.figure()
    names = locals()
    for i in range(len(loss_list)):
        names["loss%s" % (i + 1)] = torch.stack(loss_list[i], dim=0).detach().cpu().numpy()
        plt.plot(names.get("loss" + str(i + 1))[n:-1])  # , label="loss" + str(i + 1))
